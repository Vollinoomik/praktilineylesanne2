...
type SortKey = "name" | "quantity" | "status" | "createdAt";
...
const statusRank = (s: StockStatus) => (s === "OUT" ? 0 : s === "LOW" ? 1 : 2);
...
const newId = (): ProductId => "p_" + Math.random().toString(16).slice(2) + "_" + Date.now();
...
const isProduct = (x: unknown): x is Product =>
  ...
  typeof (x as any).minStock === "number" &&
...
class ProductStore {
  ...
  private commit(next: Product[]) {
    this.products = next;
    saveProducts(this.products);
  }

  add(product: Omit<Product, "id" | "createdAt">) {
    const created: Product = { ...product, id: newId(), createdAt: Date.now() };
    this.commit([created, ...this.products]);
    return created;
  }

  update(id: ProductId, patch: Partial<Pick<Product, "name" | "quantity" | "minStock">>) {
    this.commit(this.products.map(p => (p.id === id ? { ...p, ...patch } : p)));
  }

  remove(id: ProductId) { this.commit(this.products.filter(p => p.id !== id)); }
  clearAll() { this.commit([]); }
}

const applyView = (products: Product[], s: ViewState) => {
  const q = s.query.trim().toLowerCase();
  const dir = s.sortDir === "asc" ? 1 : -1;

  const key = (p: Product): string | number =>
    s.sortKey === "status" ? statusRank(getStockStatus(p))
...
function el<K extends keyof HTMLElementTagNameMap>(
  tag: K,
  opts: { className?: string; text?: string; attr?: Record<string, string> } = {},
...
const addStyle = (css: string) => document.head.appendChild(el("style", { text: css }));
...
function bindNonNegIntInput(
  ...
  input.addEventListener("change", () => {
    ...
    if (Number.isFinite(v) && v >= 0) onValid(Math.floor(v));
    else input.value = String(getFallback());
    rerender();
  });
}
...
function renderApp(root: HTMLElement, store: ProductStore) {
  setAppStyles();

  const state: ViewState = { query: "", statusFilter: "ALL", sortKey: "createdAt", sortDir: "desc" };

  const nameInput = el("input", { attr: { placeholder: "Toote nimi", type: "text" } }) as HTMLInputElement;
  const qtyInput = el("input", { attr: { placeholder: "Kogus", type: "number", min: "0", step: "1" } }) as HTMLInputElement;
  const minInput = el("input", { attr: { placeholder: "Miinimum", type: "number", min: "0", step: "1" } }) as HTMLInputElement;

  const addBtn = el("button", { className: "primary", text: "Lisa toode" }) as HTMLButtonElement;
  const clearBtn = el("button", { className: "danger", text: "Tühjenda kõik" }) as HTMLButtonElement;
  const formError = el("div", { className: "error" });

  const searchInput = el("input", { attr: { placeholder: "Otsi nime järgi…", type: "text" } }) as HTMLInputElement;
  ...
  listCard.append(
    el("div", { className: "grid header", attr: { style: "margin-bottom: 10px;" } }, [
      el("div", { text: "Toode" }),
      el("div", { text: "Kogus" }),
  ...
  const setError = (msg: string) => (formError.textContent = msg);
  ...
  const render = () => {
    ...
    const counts = products.reduce(
      ...
      { total: 0, OK: 0, LOW: 0, OUT: 0 } as { total: number; OK: number; LOW: number; OUT: number }
    );
    ...
    viewed.forEach(p => {
      ...
      const qtyEdit = el("input", { attr: { type: "number", min: "0", step: "1", value: String(p.quantity) } }) as HTMLInputElement;
      const minEdit = el("input", { attr: { type: "number", min: "0", step: "1", value: String(p.minStock) } }) as HTMLInputElement;
      ...
      listBody.appendChild(
        el("div", { className: "grid", attr: { style: "padding: 10px 0; border-top: 1px solid rgba(255,255,255,0.10);" } }, [
          el("div", {}, [
            el("div", { text: p.name }),
  ...
  addBtn.addEventListener("click", () => {
    ...
    if (!name) return setError("Palun sisesta toote nimi.");
    if (!Number.isFinite(quantity) || quantity < 0) return setError("Kogus peab olema 0 või suurem arv.");
    if (!Number.isFinite(minStock) || minStock < 0) return setError("Miinimum peab olema 0 või suurem arv.");
  ...
  statusSelect.addEventListener("change", () => { state.statusFilter = statusSelect.value as any; render(); });
  sortKeySelect.addEventListener("change", () => { state.sortKey = sortKeySelect.value as SortKey; render(); });
  sortDirSelect.addEventListener("change", () => { state.sortDir = sortDirSelect.value as SortDir; render(); });
...
if (!root) throw new Error("Puudub #app element");

const demoIfEmpty = (items: Product[]) =>
  items.length ? items : [
    { id: newId(), name: "Piim", quantity: 2, minStock: 3, createdAt: Date.now() - 3600_000 },
    { id: newId(), name: "Leib", quantity: 0, minStock: 2, createdAt: Date.now() - 1800_000 },
    { id: newId(), name: "Õunad", quantity: 10, minStock: 5, createdAt: Date.now() - 600_000 },
  ];
